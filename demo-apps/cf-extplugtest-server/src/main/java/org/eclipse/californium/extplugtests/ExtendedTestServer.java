...
public class ExtendedTestServer extends AbstractTestServer {
	...
	private static class Config extends PlugtestServer.BaseConfig {
		...
		public static class SimpleCluster {
			...
			@Option(names = "--dtls-cluster", split = ",", arity = "1..n", description = "configure DTLS-cluster-node. <dtls-interface>;<mgmt-interface>;<node-id>. --- for <dtls-interface>, for other cluster-nodes")
		...
		public static class Cluster {
			...
			@Option(names = "--k8s-dtls-cluster", description = "enable k8s DTLS-cluster mode.")
		...
		public List<InterfaceType> getInterfaceTypes() {
			if (cluster != null && cluster.k8sCluster != null) {
				external = true;
				loopback = false;
				ipv4 = true;
				ipv6 = false;
	...
	public static void main(String[] args) {
		...
		try {
			K8sManagementClient k8sGroup = null;
			NetworkConfig netConfig = NetworkConfig.createWithFile(CONFIG_FILE, CONFIG_HEADER, DEFAULTS);
			if (config.cluster != null) {
				int nodeId;
				if (config.cluster.k8sCluster != null) {
					k8sGroup = new K8sManagementClient(config.cluster.k8sCluster.cluster,
							config.cluster.k8sCluster.externalPort);
					nodeId = k8sGroup.getNodeID();
					System.out.println("dynamic k8s-cluster!");
				} else {
					nodeId = -1;
					for (ClusterNode cluster : config.cluster.simpleCluster.dtlsClusterNodes) {
					...
					if (config.cluster.simpleCluster.dtlsClusterGroup != null) {
			...
			if (k8sGroup != null) {
				server.addClusterEndpoint(secondaryExecutor, config.cluster.k8sCluster.dtls, k8sGroup.getNodeID(), null,
						k8sGroup, config);
			} else if (config.cluster != null && config.cluster.simpleCluster != null) {
				...
				if (config.cluster.simpleCluster.dtlsClusterGroup == null) {
					nodes = new DtlsClusterConnector.ClusterNodesProvider() {
						...
						public InetSocketAddress getClusterNode(int nodeId) {
							for (ClusterNode node : config.cluster.simpleCluster.dtlsClusterNodes) {
				...
				for (ClusterNode cluster : config.cluster.simpleCluster.dtlsClusterNodes) {
					if (cluster.dtls != null) {
						if (config.cluster.simpleCluster.dtlsClusterGroup != null) {
							group = new ClusterGroup(config.cluster.simpleCluster.dtlsClusterGroup, cluster.cluster);
						}
						server.addClusterEndpoint(secondaryExecutor, cluster.dtls, cluster.nodeId, nodes, group, config);
					}
				}
			}
	...
	private void addClusterEndpoint(ScheduledExecutorService secondaryExecutor, InetSocketAddress dtlsInterface,
			int nodeId, DtlsClusterConnector.ClusterNodesProvider nodesProvider,
			DtlsClusterConnector.ClusterNodesDiscover nodesDiscoverer, BaseConfig cliConfig) {
